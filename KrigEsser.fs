...
let esser () = 
  ...
  shuffle c
  let (d1,d2) = List.splitAt ((c.Length/2)-2) (Array.toList c)
  let newd1 = List.toArray (d1@[14;14;14;14])
  shuffle (newd1)
  ((Array.toList newd1), d2)
...
let rec doFight (d1: LinkedList<int>) (d2: LinkedList<int>) (winnings: int list)= 
  ...
  with
    | :? System.NullReferenceException -> Console.WriteLine("\n")
                                          let s = 
                                            if(d1.Count = 0) then "Spiller 2 har vundet selvom de ikke havde nogen esser fra begyndelsen!"
                                            else "Spiller 1 har vundet ved at have alle esserne fra begyndelsen!"
                                          printfn "%s" s
